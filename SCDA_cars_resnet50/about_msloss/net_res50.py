...
class ResNet_circle_loss_pairwise(nn.Module):

    def __init__(self, block, layers,mode=None,  zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNet_circle_loss_pairwise, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
...
def _resnet_circle_loss_pairwise( block, layers,mode, **kwargs):
    model = ResNet_circle_loss_pairwise(block, layers,mode, **kwargs)
    return model
...
def FGIAnet100_metric5_circle_loss_pairwise(**kwargs):
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet_circle_loss_pairwise(Bottleneck, [3, 4, 6, 3],'ms_loss',
                   **kwargs)
